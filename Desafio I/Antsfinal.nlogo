;; Definição das raças
breed [formigas formiga]  ;; "formigas" é o nome da raça e "formiga" é o nome do tipo de agente
breed [predadores predador] ;; "predadores" é o nome da raça e "predador" é o nome do tipo de agente
...
formigas-own [
  is-predator?    ;; Indica se é um predador (true) ou uma formiga (false) - esta variável é desnecessária para predadores
]
...
to setup
  clear-all
  set-default-shape formigas "bug" ;; Forma para formigas
  set-default-shape predadores "bug" ;; Forma para predadores
...
to hunt
  let target one-of formigas  ;; Seleciona uma formiga aleatória
  if target != nobody [
    face target
    move-to target
